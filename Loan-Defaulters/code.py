...
plt.bar(fully_paid.index, fully_paid.values, alpha=0.8)
...
X_train['int.rate'] = X_train['int.rate'].apply(lambda x: x.replace(x[-1],''))
X_train['int.rate'] = X_train['int.rate'].apply(lambda x: float(x)/100)

X_test['int.rate'] = X_test['int.rate'].apply(lambda x: x.replace(x[-1],''))
X_test['int.rate'] = X_test['int.rate'].apply(lambda x: float(x)/100)
...
for i in range(9):
    sns.boxplot(x=y_train,y=num_df[cols[i]],ax=axes[i],saturation = 3)
# Code ends here
